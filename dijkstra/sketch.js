...
function setup()
{
		...
		class Node
	{
		...
		is_path()
		{
			return (this.color == PATH_COLOR||this.color == DARK_PATH_COLOR) ? true : false;
		}
		is_weight()
		{
		...
		{
			let neighbors = []
			if (this.row>0 && !boxes[this.row-1][this.col].is_barrier())neighbors.push(boxes[this.row-1][this.col]);
			if (this.col>0 && !boxes[this.row][this.col-1].is_barrier())neighbors.push(boxes[this.row][this.col-1]);
			if (this.row<total_rows-1 && !boxes[this.row+1][this.col].is_barrier())neighbors.push(boxes[this.row+1][this.col]);
			if (this.col<total_rows-1 && !boxes[this.row][this.col+1].is_barrier())neighbors.push(boxes[this.row][this.col+1]);
			return neighbors;
		}
		get_neighbors_maze()
		{
			...
			if (this.row<total_rows-2)neighbors.push(boxes[this.row+2][this.col]);
			if (this.col<total_rows-2)neighbors.push(boxes[this.row][this.col+2]);
			return neighbors;
		}
	}
...
function clearearlier() {
    ...
    for (let x = 0; x < width; x += cell_width)
    {
        if (boxes[row][col].is_close()||boxes[row][col].is_open()||boxes[row][col].is_path())
...
async function DFS_Maze_generator(start,end) {
    ...
    if (not_visited_neighbors.length!=0) {
        maze.push(current);
        let to_be_added = not_visited_neighbors[Math.floor(Math.random()*not_visited_neighbors.length)];
        boxes[(to_be_added.id[0]+current.id[0])/2][(to_be_added.id[1]+current.id[1])/2].make_visited();
